...
__fish_brew_complete_cmd 'bundle' 'Bundler for non-Ruby dependencies from Homebrew, Homebrew Cask, Mac App Store, Visual Studio Code (and forks/variants) and Go packages'
...
__fish_brew_complete_arg 'test-bot' -l only-cleanup-after -d 'Only run the post-cleanup step. Needs `--cleanup`'
__fish_brew_complete_arg 'test-bot' -l only-cleanup-before -d 'Only run the pre-cleanup step. Needs `--cleanup`'
