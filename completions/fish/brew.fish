...
__fish_brew_complete_cmd 'bundle' 'Bundler for non-Ruby dependencies from Homebrew, Homebrew Cask, Mac App Store, Visual Studio Code (and forks/variants), Go packages and Flatpak'
...
__fish_brew_complete_arg 'bundle' -l flatpak-remotes -d '`list` or `dump` Flatpak remotes'
...
__fish_brew_complete_arg 'bundle' -l no-flatpak-remotes -d '`dump` without Flatpak remotes. Enabled by default if `$HOMEBREW_BUNDLE_DUMP_NO_FLATPAK_REMOTES` is set'
