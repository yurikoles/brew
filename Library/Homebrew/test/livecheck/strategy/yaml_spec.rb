...
RSpec.describe Homebrew::Livecheck::Strategy::Yaml do
  ...
  describe "::versions_from_content" do
    ...
    it "returns an array of version strings when given content and a block" do
      ...
      expect(yaml.versions_from_content(content_simple) { |yaml| yaml["version"] }).to eq(matches[:simple])
  ...
  describe "::find_versions" do
    ...
    it "finds versions in content using a block" do
      expect(yaml.find_versions(url: http_url, regex:, content:) do |yaml, regex|
        yaml["versions"].select { |item| item["version"]&.match?(regex) }
                        .map { |item| item["version"][regex, 1] }
      ...
      expect(yaml.find_versions(url: http_url, content:) do |yaml|
        regex = /^v?(\d+(?:\.\d+)+)$/i
        yaml["versions"].select { |item| item["version"]&.match?(regex) }
                        .map { |item| item["version"][regex, 1] }
